...
$query = '
    ...
    FROM 
        s2db.sbds_tx_account_witness_votes 
    WHERE 
        `timestamp` >= :from AND
        `timestamp` < :to        
';
...
foreach ($years as $year) {
    for ($i = 1; $i <= 12; $i++) {
        ...
        $cache = md5('cache-witness-votes-'.$year.'-'.$i);
...
<div class="col-lg-6">
    <div class="card">
        <div class="card-header">
            <h3 class="card-title">Witness Vote Activity</h3>
        </div>

        <canvas id="witness-votes" height="360"></canvas>

        <script>
            new window.Chart(document.getElementById('witness-votes'), {
                type   : 'line',
                data   : {
